...
function searchNameServerExample() {
	...
	try {
		 $response = $ascioClient->SearchNameServer(new ascio\SearchNameServer($sessionId, $criteria));
        } catch (\Exception $e) {
    		echo ("[".$e->faultcode . "] ". $e->faultstring);
	    	return;
        
        }
	$result = $response->getSearchNameServerResult();
	echo "Code: ".$result->getResultCode()."\n";
	echo "Message: ".$result->getMessage()."\n";  
	return $result;
}
searchNameServerExample();