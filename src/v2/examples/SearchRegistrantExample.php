...
function searchRegistrantExample() {
	...
	try {
		 $response = $ascioClient->SearchRegistrant(new ascio\SearchRegistrant($sessionId, $criteria));
        } catch (\Exception $e) {
    		echo ("[".$e->faultcode . "] ". $e->faultstring);
	    	return;
        
        }
	$result = $response->getSearchRegistrantResult();
	echo "Code: ".$result->getResultCode()."\n";
	echo "Message: ".$result->getMessage()."\n";  
	return $result;
}
searchRegistrantExample();